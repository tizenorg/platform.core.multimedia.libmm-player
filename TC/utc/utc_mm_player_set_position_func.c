...
#include "utc_mm_player_set_position_func.h"

struct tet_testlist tet_testlist[] = {
	{utc_mm_player_set_position_func_01, 1},
	{utc_mm_player_set_position_func_02, 2},
	{NULL, 0}
};

GMainLoop *g_set_pos_loop;
bool g_is_set_pos_positive = 0;
		...
		case MM_MESSAGE_STATE_CHANGED:
			switch(param->state.current)
			{
				...
				case MM_PLAYER_STATE_PLAYING:
					...
					int pos = 4000; // 4sec
					if (g_is_set_pos_positive)
					{
						
						ret = mm_player_set_position(player, MM_PLAYER_POS_FORMAT_TIME, pos);
						dts_check_eq(__func__, ret, MM_ERROR_NONE, "err=%x", ret );
					}
					else
					{
						ret = mm_player_set_position(NULL, MM_PLAYER_POS_FORMAT_TIME, pos);
						dts_check_ne(__func__, ret, MM_ERROR_NONE, "err=%x", ret );
					}

					// XX_fucn_01 can return when g_main_loop is quit.
					UTC_MM_PLAYER_QUIT_LOOP(g_set_pos_loop);
...
/**
* @brief	This tests int mm_player_set_position() API with valid parameter
* 		Create a player handle with valid parameter & Test the handle by playing
* @par ID	utc_mm_player_set_position_func_01
* @param	[in] &player = handle of player to be populated
* @return	This function returns zero on success, or negative value with error code
*/
void utc_mm_player_set_position_func_01()
{
	MMHandleType player = 0;
	int ret = 0;
	...
	UTC_MM_PLAYER_RUN_LOOP(g_set_pos_loop);
...
/**
* @brief 		This tests int mm_player_set_position() API with invalid parameter
* 			Create a player handle with a NULL out param
* @par ID	utc_mm_player_set_position_func_02
* @param	[in] &player = NULL
* @return	error code on success 
*/
void utc_mm_player_set_position_func_02()
{	
	MMHandleType player = 0;
	int ret = 0;
	int pos = 0;
