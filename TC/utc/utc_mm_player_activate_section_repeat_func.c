...
#include "utc_mm_player_activate_section_repeat_func.h"

struct tet_testlist tet_testlist[] = {
	{utc_mm_player_activate_section_repeat_func_01, 1},
	{utc_mm_player_activate_section_repeat_func_02, 2},	
	{NULL, 0}
};

GMainLoop *g_activate_repeat_loop;
		...
		case MM_MESSAGE_STATE_CHANGED:
			switch(param->state.current)
			{
				...
				case MM_PLAYER_STATE_PLAYING:
					...
					ret = mm_player_activate_section_repeat(player, 1000, 3000);
					dts_check_eq(__func__, ret, MM_ERROR_NONE, "err=%x", ret );
					
					// XX_fucn_01 can return when g_main_loop is quit.
					UTC_MM_PLAYER_QUIT_LOOP(g_activate_repeat_loop);
					break;
...
/**
* @brief	This tests int mm_player_activate_section_repeat() API with valid parameter
* 		Create a player handle with valid parameter & Test the handle by playing
* @par ID	utc_mm_player_activate_section_repeat_func_01
* @param	[in] &player = handle of player to be populated
* @return	This function returns zero on success, or negative value with error code
*/
void utc_mm_player_activate_section_repeat_func_01()
{
	int cur_pos = 0;
	int offset = 4000;	// 4sec
	MMHandleType player = 0;
	int ret = 0;
	
	UTC_MM_PLAYER_CREATE(&player, ret);	
	mm_player_set_attribute(player, NULL, "profile_uri", MP3_FILE, strlen(MP3_FILE), NULL);
	...
	UTC_MM_PLAYER_RUN_LOOP(g_activate_repeat_loop);
...
/**
* @brief 		This tests int mm_player_activate_section_repeat() API with invalid parameter
* 			Create a player handle with a NULL out param
* @par ID	utc_mm_player_activate_section_repeat_func_02
* @param	[in] &player = NULL
* @return	error code on success 
*/
void utc_mm_player_activate_section_repeat_func_02()
{	
	MMHandleType player = 0;
	int start_pos = -10; // invalid pos 
	int end_pos = 5;
	int ret = 0;	
	...
	ret = mm_player_activate_section_repeat(player, start_pos, end_pos);
